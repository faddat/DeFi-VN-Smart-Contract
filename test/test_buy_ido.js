...
contract('IdoDFY contract: Buy IDO', function (accounts) {
    ...
    let ETHContract, ETHContractAddress, BTCContact, BTCContractAddress, BNBContractAddress
    ...
    before("setup", async function () {
        ...
        BNBContractAddress='0x0000000000000000000000000000000000000000'
    ...
    //             "Returned error: VM Exception while processing transaction: revert Amount DFY request is too low -- Reason given: Amount DFY request is too low.",
    ...
    it("Buy Ido success by BNB", async () => {
        await idoDFYContract.updateExchangePair(BNBContractAddress, 2000, 1, {from: owner})
        await idoDFYContract.setStage(0, { from: owner })

        const buyAmount = BigNumber(0.5*Math.pow(10, 18))
        const ethValue = web3.utils.toWei("0.5", "ether");
        // await ETHContract.approve(idoDFYContractAddress, buyAmount, {from: ownerETH})
        try {
            await idoDFYContract.buyIdo(BNBContractAddress, buyAmount, address0, {from: ownerETH, value: ethValue})
        } catch (error) {
            console.log(error)
        }

        const ownerETHBalance = await DFYContract.balanceOf(ownerETH, { from: ownerETH })
        assert.equal(ownerETHBalance.toString(), "1000000000000000000000", "Buy IDO with exact 1000 DFY success")
        
        const balance = await web3.eth.getBalance(idoDFYContract.address); //Will give value in.
        // const idoDFYbalance = web3.utils.toDecimal(balance);
        // console.log(balance/1e18)
        assert.equal(ethValue,balance,"BNB balance invalid")

        const ownerBalanceBefore=await web3.eth.getBalance(owner)
        await idoDFYContract.withdrawnBNB({from: owner})
        const ownerBalanceAfter=await web3.eth.getBalance(owner)
        // assert.equal(ownerBalanceBefore.toNumber()+ethValue,ownerBalanceAfter.toNumber(),"BNB balance invalid")
